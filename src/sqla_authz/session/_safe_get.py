...
async def async_safe_get(
    ...
    *,
    ...
    """Async version of :func:`safe_get`.
    ...
    Args:
        session: The async SQLAlchemy session to load from.
        entity_class: The mapped model class to load.
        pk: The primary key value.
        actor: The actor performing the action.
        action: The action to check (default ``"read"``).
    ...
    Example::
        ...
        post = await async_safe_get(session, Post, 42, actor=current_user)
        if post is None:
            raise HTTPException(404)
    """
...
async def async_safe_get_or_raise(
    session: AsyncSession,
    entity_class: type[T],
    pk: Any,
    *,
    ...
    """Async version of :func:`safe_get_or_raise`.
    ...
    Example::
        ...
        post = await async_safe_get_or_raise(session, Post, 42, actor=current_user)
        if post is None:
            raise HTTPException(404)
    """
    obj = await session.get(entity_class, pk)
